...
const STORE_COLORS = [
  "bg-blue-50 text-blue-800 border-blue-200", "bg-emerald-50 text-emerald-800 border-emerald-200",
  "bg-amber-50 text-amber-800 border-amber-200", "bg-rose-50 text-rose-800 border-rose-200",
  "bg-purple-50 text-purple-800 border-purple-200", "bg-cyan-50 text-cyan-800 border-cyan-200",
...
export default function ExpiryManager() {
  ...
  useEffect(() => {
    ...
    try {
      ...
      firebaseAuth.signInAnonymously().then(() => {
        ...
        const unsubscribeProducts = firestoreDb.collection("stores").doc(auth.store).collection("products").onSnapshot(
          (snapshot) => {
        ...
        const unsubscribeSettings = firestoreDb.collection("stores").doc(auth.store).collection("settings").doc("config").onSnapshot((docSnap) => {
          if (docSnap.exists) {
        ...
        return () => { unsubscribeProducts(); unsubscribeSettings(); };
      });
  ...
  const getExpiryStatus = (expiryDate, reminderDays = 60, hasSecondReminder = false, reminderDays2 = 14) => {
    ...
    if (diffDays <= dangerThreshold) return { status: "danger", label: "高度警戒", color: "text-red-600", bg: "bg-red-50", bgBar: "bg-red-400", border: "border-red-200", days: diffDays };
    if (diffDays <= reminderDays) return { status: "warning", label: "中度警戒", color: "text-orange-600", bg: "bg-orange-50", bgBar: "bg-orange-400", border: "border-orange-200", days: diffDays };
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    const dataToSave = {
      ...formData, name: formData.name.trim(), barcode: formData.barcode.trim(),
    ...
    if (useLocalMode) {
      ...
      if (dataToSave.barcode) {
        batch.set(db.collection("master_products").doc(dataToSave.barcode.replace(/\//g, "_")), {
          name: dataToSave.name, category: dataToSave.category, reminderDays: dataToSave.reminderDays,
          hasSecondReminder: dataToSave.hasSecondReminder, reminderDays2: dataToSave.reminderDays2, updatedAt: dataToSave.updatedAt
        }, { merge: true });
      }
      await batch.commit();
      syncSnapshotToGoogleSheets(updatedProducts);
  ...
  const handleQuantityMinus = async (product) => {
    ...
    const updatedProduct = { ...product, quantity: currentQty - 1 };
    ...
    if (useLocalMode) {
      ...
      await db.collection("stores").doc(auth.store).collection("products").doc(product.id).update({ quantity: currentQty - 1 });
      syncSnapshotToGoogleSheets(newProductsList);
    }
  };
  ...
  const handleBulkDelete = () => {
    ...
    confirmAction(`確定要刪除選取的 ${selectedIds.size} 筆庫存嗎？`, async () => {
      ...
      if (useLocalMode) {
        ...
        const batch = db.batch();
        selectedIds.forEach((id) => { batch.delete(db.collection("stores").doc(auth.store).collection("products").doc(id)); });
        await batch.commit();
      }
  ...
  let filteredProducts = products.filter((p) => {
    ...
    if (filterStatus !== "all") {
      ...
      if (filterStatus === "warning" && st.status !== "danger" && st.status !== "warning") return false;
  ...
  products.forEach((p) => {
    ...
    if (diff >= 0) {
      ...
      const current = locationBarcodeEarliest[locKey];
      if (!current || new Date(p.expiryDate).getTime() < new Date(current.expiryDate).getTime()) {
        locationBarcodeEarliest[locKey] = { id: p.id, expiryDate: p.expiryDate };
      }
    }
  });
  Object.values(locationBarcodeEarliest).forEach((item) => fifoIds.add(item.id));
  ...
  const renderCalendar = () => {
    ...
    for (let d = 1; d <= daysInMonth; d++) {
      ...
      days.push(
        <button key={d} onClick={() => setSelectedCalendarDay(dateStr)} className={`p-2 border rounded-xl flex flex-col items-center justify-start h-14 relative transition ${isSelected ? "ring-2 ring-[#0058a3] bg-blue-50" : "bg-white hover:bg-gray-50 border-gray-200"} ${dateStr === getTodayStr() ? "border-[#FBD914] border-2 font-bold" : ""}`}>
      ...
      <div className="min-h-screen bg-[#0058a3] flex items-center justify-center p-4">
        <div className="bg-white w-full max-w-sm rounded-3xl shadow-2xl overflow-hidden border-b-8 border-[#FBD914]">
          <div className="p-8 text-center bg-white">
            <h1 className="text-2xl font-black text-[#0058a3] tracking-wider mb-6 flex items-center justify-center gap-2"><Package className="w-8 h-8 text-[#FBD914]"/> 向即期品說再見</h1>
            {!auth.store ? (
              <div className="grid grid-cols-2 gap-3">
                {stores.map((store, i) => (
                  <button key={store} onClick={() => setAuth(prev => ({ ...prev, store }))} className={`p-3 border-2 rounded-2xl font-bold text-lg transition-transform active:scale-95 ${STORE_COLORS[i % STORE_COLORS.length]}`}>{store}</button>
                ))}
              </div>
            ) : (
              <form onSubmit={handleLogin} className="space-y-4 animate-in fade-in">
                <div className="flex justify-between items-center mb-4"><button type="button" onClick={handleLogout} className="text-gray-400 text-sm"><ChevronLeft className="w-4 h-4 inline"/> 更換</button><span className="font-bold text-[#0058a3] bg-blue-50 px-3 py-1 rounded">{auth.store}店</span></div>
                <input type="password" placeholder="輸入密碼..." required value={auth.password} onChange={(e) => setAuth(prev => ({ ...prev, password: e.target.value }))} className="w-full px-4 py-4 border-2 rounded-2xl font-bold text-center text-lg outline-none focus:border-[#0058a3] bg-gray-50 transition" autoFocus />
                <button type="submit" className="w-full py-4 bg-[#0058a3] text-[#FBD914] font-black rounded-xl text-lg hover:bg-[#004a89] transition shadow-md">登 入</button>
              </form>
            )}
          </div>
        </div>
    ...
    <div className="min-h-screen bg-slate-50 font-sans pb-24 relative overflow-x-hidden">
        ...
        <div className="fixed inset-0 bg-slate-900/60 backdrop-blur-sm z-[9999] flex items-center justify-center p-4">
          <div className="bg-white rounded-2xl p-6 w-full max-w-sm shadow-2xl border-t-8 border-[#0058a3]">
            ...
            <div className="flex gap-3">
              <button onClick={() => setConfirmDialog(null)} className="flex-1 py-2.5 rounded-xl border border-gray-300 font-bold text-slate-600">取消</button>
              <button onClick={() => { confirmDialog.onConfirm(); setConfirmDialog(null); }} className="flex-1 py-2.5 rounded-xl bg-[#0058a3] text-white font-bold">確認</button>
            </div>
          </div>
        </div>
      )}
      {isScannerOpen && (
        <div className="fixed inset-0 z-[999] bg-black flex flex-col items-center justify-center">
          <button onClick={handleStopScanner} className="absolute top-6 right-6 z-10 bg-white/20 p-3 rounded-full text-white"><X className="w-8 h-8" /></button>
          <div id="reader" className="w-full max-w-sm bg-gray-900 overflow-hidden shadow-2xl rounded-2xl border-4 border-[#FBD914]"></div>
          <p className="text-white mt-8 font-bold tracking-widest animate-pulse flex items-center gap-2"><Camera className="w-5 h-5" /> 請對準條碼</p>
        </div>
      )}
        ...
        <div className="flex items-center gap-3 self-start sm:self-auto">
          ...
          <div>
            ...
            <div className="flex items-center gap-2 mt-0.5">
              <span className="text-[10px] bg-white/20 text-white px-2 py-0.5 rounded font-bold">{auth.store}店 ({auth.role === "admin" ? "管理" : "一般"})</span>
              <button onClick={handleLogout} className="text-white/70 hover:text-white"><LogOut className="w-3.5 h-3.5" /></button>
            </div>
          </div>
        </div>
        <div className="flex gap-2 w-full sm:w-auto justify-between sm:justify-end">
          <button onClick={() => setIsCalendarOpen(true)} className="p-2.5 bg-white/10 text-white rounded-xl"><CalendarDays className="w-5 h-5" /></button>
          {auth.role === "admin" && (
            <>
          ...
          <button onClick={() => { setFormData(defaultForm); setEditingId(null); setIsModalOpen(true); }} className="px-4 py-2.5 bg-[#FBD914] text-[#0058a3] rounded-xl font-black flex items-center gap-1 shadow-lg border-b-4 border-[#d4b50d] active:border-b-0"><Plus className="w-5 h-5" /> 新增商品</button>
        </div>
      </header>
      ...
      <main className="max-w-3xl mx-auto px-4 py-6">
        ...
        <div className="flex flex-col gap-4 mb-6">
          <div className="flex gap-2 bg-gray-200 p-1.5 rounded-xl w-full overflow-x-auto custom-scrollbar">
            <button onClick={() => { setFilterCategory("all"); setCurrentPage(1); }} className={`flex-shrink-0 px-4 py-2 text-sm font-bold rounded-lg ${filterCategory === "all" ? "bg-white text-slate-800 shadow-sm" : "text-gray-500"}`}>全部</button>
            <button onClick={() => { setFilterCategory("room_temp"); setCurrentPage(1); }} className={`flex-shrink-0 px-4 py-2 text-sm font-bold rounded-lg flex items-center gap-1 ${filterCategory === "room_temp" ? "bg-white text-orange-600 shadow-sm" : "text-gray-500"}`}><Sun className="w-4 h-4" /> 常溫</button>
            <button onClick={() => { setFilterCategory("frozen"); setCurrentPage(1); }} className={`flex-shrink-0 px-4 py-2 text-sm font-bold rounded-lg flex items-center gap-1 ${filterCategory === "frozen" ? "bg-white text-[#0058a3] shadow-sm" : "text-gray-500"}`}><Snowflake className="w-4 h-4" /> 冷凍</button>
          </div>
          <div className="flex gap-2 flex-col sm:flex-row">
            <div className="relative flex-1">
              <Search className="absolute left-4 top-1/2 -translate-y-1/2 text-gray-400 w-5 h-5" />
              <input type="text" placeholder="搜尋名稱或條碼..." value={searchQuery} onChange={(e) => { setSearchQuery(e.target.value); setCurrentPage(1); }} className="w-full pl-12 pr-14 py-3 bg-white border-2 border-gray-200 rounded-xl text-sm font-bold focus:border-[#0058a3] outline-none" />
              <button onClick={() => handleStartScanner("search")} className="absolute right-2 top-1/2 -translate-y-1/2 p-2 bg-blue-50 text-[#0058a3] rounded-lg"><Camera className="w-4 h-4" /></button>
            </div>
            <div className="flex gap-2">
              ...
              <button onClick={() => setSortOrder(prev => prev === "asc" ? "desc" : "asc")} className="px-3 bg-white border-2 border-gray-200 rounded-xl text-gray-500 flex items-center justify-center"><ArrowUpDown className={`w-5 h-5 ${sortOrder === "desc" ? "rotate-180" : ""}`}/></button>
            </div>
          </div>
        </div>
        ...
        {auth.role === "admin" && (
          <div className="flex justify-between items-center mb-4 bg-white p-2.5 rounded-xl border-2 border-gray-200 shadow-sm">
            <label className="flex items-center gap-2 cursor-pointer font-bold text-sm text-slate-600">
              <input type="checkbox" checked={currentDisplayIds.length > 0 && selectedIds.size === currentDisplayIds.length} onChange={toggleSelectAll} className="w-4 h-4 accent-[#0058a3] rounded" />
              全選本頁 ({selectedIds.size > 0 ? `已選 ${selectedIds.size}` : `共 ${displayList.length} 組`})
            </label>
            {selectedIds.size > 0 && <button onClick={handleBulkDelete} className="px-4 py-1.5 bg-red-600 text-white text-sm font-bold rounded-lg shadow-sm hover:bg-red-700 flex items-center gap-1"><Trash2 className="w-4 h-4" /> 大量刪除</button>}
          </div>
        )}

        {loading ? <div className="text-center py-20 text-gray-400 font-bold"><Loader2 className="w-10 h-10 animate-spin mx-auto text-[#0058a3]" /> 載入中...</div> 
         : paginatedList.length === 0 ? <div className="text-center py-20 bg-white rounded-3xl border-2 border-dashed border-gray-200 text-gray-400 font-bold tracking-widest">目前條件下沒有商品唷！</div>
         : (
              ...
              <div key={group.key} className="bg-white rounded-2xl shadow-sm border-2 border-gray-200 overflow-hidden relative">
                <div className="bg-slate-50 p-4 border-b flex justify-between items-center">
                  <div className="flex-1">
                    <h3 className="font-black text-slate-800 text-lg leading-tight flex items-center gap-2"><Package className="w-5 h-5 text-gray-400" /> {group.name}</h3>
                    <div className="flex items-center gap-2 mt-1.5">
                      <span className={`text-[10px] px-2 py-0.5 rounded font-black tracking-widest ${group.category === "frozen" ? "bg-[#0058a3]/10 text-[#0058a3]" : "bg-orange-100 text-orange-700"}`}>{group.category === "frozen" ? "冷凍" : "常溫"}</span>
                      <div className="text-xs text-slate-500 font-mono"><Barcode className="w-3.5 h-3.5 inline mr-1" />{group.barcode}</div>
                    </div>
                  </div>
                  ...
                  {group.batches.map((product) => {
                    ...
                    return (
                      <div key={product.id} className={`flex flex-col sm:flex-row sm:items-center justify-between p-3 rounded-xl border-2 relative transition ${isSoldOut ? "bg-gray-100 border-gray-300 opacity-70" : isFIFO ? "border-[#0058a3] bg-blue-50/30" : status.border}`}>
                        {isFIFO && !isSoldOut && <div className="absolute -top-3 -right-2 z-[10]"><span className="bg-[#0058a3] text-[#FBD914] text-[10px] font-black px-2.5 py-1 rounded-full shadow-md animate-pulse border-2 border-white">🏷️ 此地點優先使用</span></div>}
                        <div className="flex items-center gap-3 flex-1 mb-3 sm:mb-0">
                          {auth.role === "admin" && <input type="checkbox" checked={selectedIds.has(product.id)} onChange={() => { const next = new Set(selectedIds); if (next.has(product.id)) next.delete(product.id); else next.add(product.id); setSelectedIds(next); }} className="w-4 h-4 accent-[#0058a3] rounded" />}
                          <div className="flex flex-col gap-1">
                            ...
                            <div className="text-xs text-gray-500 flex gap-2"><span>進: {product.receiveDate}</span> <span className="font-bold text-slate-700">期: {product.expiryDate}</span></div>
                          </div>
                        </div>
                        <div className="flex items-center justify-between sm:justify-end gap-3 w-full sm:w-auto">
                          {!isSoldOut && (
                            <div className={`px-3 py-1.5 rounded-lg ${status.bg} ${status.color} font-black text-xs flex flex-col items-center justify-center min-w-[70px] shadow-sm`}>
                          ...
                          <div className="flex gap-1.5">
                            {!isSoldOut && (
                              <>
                                <button onClick={() => handleQuantityMinus(product)} className="p-2 text-emerald-600 bg-emerald-50 hover:bg-emerald-100 rounded-lg flex items-center gap-1"><Minus className="w-4 h-4" /><span className="text-xs font-bold">1</span></button>
        ...
        {totalPages > 1 && (
          <div className="flex justify-center items-center gap-4 mt-8">
            <button onClick={() => setCurrentPage(p => Math.max(1, p - 1))} disabled={currentPage === 1} className="p-2 bg-white border-2 border-gray-200 rounded-xl disabled:opacity-50"><ChevronLeft className="w-6 h-6" /></button>
            <span className="font-bold text-slate-600">第 {currentPage} 頁，共 {totalPages} 頁</span>
            <button onClick={() => setCurrentPage(p => Math.min(totalPages, p + 1))} disabled={currentPage === totalPages} className="p-2 bg-white border-2 border-gray-200 rounded-xl disabled:opacity-50"><ChevronRight className="w-6 h-6" /></button>
          </div>
        )}
      </main>
      ...
      {isSettingsOpen && (
        <div className="fixed inset-0 bg-slate-900/60 backdrop-blur-sm z-50 flex items-center justify-center p-4">
          <div className="bg-white w-full max-w-sm rounded-3xl shadow-2xl p-6 border-t-8 border-[#0058a3] max-h-[90vh] overflow-y-auto">
            <div className="flex justify-between items-center mb-6"><h2 className="text-xl font-black flex items-center gap-2 text-[#0058a3]"><Settings className="w-6 h-6" /> 店鋪管理設定</h2><button onClick={() => setIsSettingsOpen(false)} className="p-2 bg-gray-100 rounded-full"><X className="w-5 h-5" /></button></div>
            <div className="space-y-3 mb-6">
              <button onClick={handleBulkSyncToSheet} disabled={isSyncing} className="w-full py-2.5 bg-blue-50 text-[#0058a3] border border-blue-200 rounded-xl font-bold flex items-center justify-center gap-2">
            ...
            <div className="flex gap-2 mb-4">
              <input value={newLocationInput} onChange={(e) => setNewLocationInput(e.target.value)} placeholder="輸入新地點名稱..." className="flex-1 px-3 py-2 border-2 rounded-xl text-sm font-bold outline-none" />
              <button onClick={() => { if(!newLocationInput.trim() || locations.includes(newLocationInput.trim())) return; const updated = [...locations, newLocationInput.trim()]; setLocations(updated); if(useLocalMode) localStorage.setItem(`expiry_manager_settings_${auth.store}`, JSON.stringify({locations: updated})); else db.collection("stores").doc(auth.store).collection("settings").doc("config").set({locations: updated}, {merge:true}); setNewLocationInput(""); showToast("新增成功");}} disabled={!newLocationInput.trim()} className="px-4 py-2 bg-[#0058a3] text-[#FBD914] font-bold rounded-xl">新增</button>
            </div>
            <div className="max-h-[20vh] overflow-y-auto space-y-2 mb-6">
              {locations.map((loc) => (<div key={loc} className="flex justify-between items-center bg-slate-50 p-2.5 rounded-xl border"><span className="text-sm font-bold">{loc}</span><button onClick={() => confirmAction(`刪除地點 ${loc}?`, () => { const updated = locations.filter(l=>l!==loc); setLocations(updated); if(useLocalMode) localStorage.setItem(`expiry_manager_settings_${auth.store}`, JSON.stringify({locations: updated})); else db.collection("stores").doc(auth.store).collection("settings").doc("config").set({locations: updated}, {merge:true}); })} className="p-1.5 text-red-400"><Trash2 className="w-4 h-4" /></button></div>))}
            </div>
            <h3 className="font-bold text-sm text-slate-700 mb-2 border-b pb-1">安全設定</h3>
            {!isEditingPassword ? (
              <button onClick={() => setIsEditingPassword(true)} className="w-full py-2.5 border-2 rounded-xl text-slate-600 font-bold">修改各級密碼</button>
            ) : (
      ...
      {isCalendarOpen && (
        <div className="fixed inset-0 bg-slate-900/80 backdrop-blur-sm z-50 flex flex-col items-center justify-center p-4">
          <div className="bg-white w-full max-w-md rounded-3xl shadow-2xl flex flex-col overflow-hidden max-h-[90vh]">
            <div className="p-4 bg-[#0058a3] text-white flex justify-between items-center"><h2 className="text-xl font-black flex items-center gap-2"><CalendarDays className="w-6 h-6 text-[#FBD914]" /> 效期日曆</h2><button onClick={() => setIsCalendarOpen(false)} className="p-2 hover:bg-white/20 rounded-full"><X className="w-6 h-6" /></button></div>
            <div className="p-4 flex-1 overflow-y-auto">
              <div className="flex justify-between items-center mb-4 px-2">
                <button onClick={() => setCalendarDate(new Date(calendarDate.getFullYear(), calendarDate.getMonth() - 1, 1))} className="p-2 bg-gray-100 rounded-full"><ChevronLeft className="w-5 h-5" /></button>
                <h3 className="font-black text-lg text-[#0058a3]">{calendarDate.getFullYear()} 年 {calendarDate.getMonth() + 1} 月</h3>
                <button onClick={() => setCalendarDate(new Date(calendarDate.getFullYear(), calendarDate.getMonth() + 1, 1))} className="p-2 bg-gray-100 rounded-full"><ChevronRight className="w-5 h-5" /></button>
              </div>
              <div className="grid grid-cols-7 gap-1 mb-2 text-center text-xs font-black text-gray-400"><div>日</div><div>一</div><div>二</div><div>三</div><div>四</div><div>五</div><div>六</div></div>
      ...
      {isModalOpen && (
        <div className="fixed inset-0 bg-slate-900/60 backdrop-blur-sm z-[80] flex flex-col items-center justify-start pt-[8vh] p-4 overflow-y-auto">
          <div className="bg-white w-full max-w-md rounded-3xl shadow-2xl flex flex-col overflow-hidden mb-10 border border-gray-100">
            <div className="flex justify-between items-center p-5 border-b bg-gray-50"><h2 className="font-black text-[#0058a3] flex items-center gap-2 text-xl"><Package className="w-6 h-6 text-[#FBD914]" /> {editingId ? "編輯商品" : "新增商品"}</h2><button onClick={closeModal} className="p-2 text-gray-400 hover:bg-gray-200 rounded-full"><X className="w-6 h-6" /></button></div>
              ...
              <form id="productForm" onSubmit={handleSubmit} className="space-y-5">
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                  <div>
                    ...
                    <div className="flex gap-2">
                      <input type="text" name="barcode" required value={formData.barcode} onChange={handleInputChange} disabled={auth.role !== "admin" && editingId} className="w-full px-3 py-2 border-2 rounded-xl text-sm font-bold focus:border-[#0058a3] outline-none min-w-0" />
                      <button type="button" onClick={() => handleStartScanner("form")} disabled={auth.role !== "admin" && editingId} className="px-3 py-2 bg-blue-50 text-[#0058a3] rounded-xl border border-blue-200"><Camera className="w-5 h-5" /></button>
                    </div>
                  </div>
                  <div>
                    <label className="block text-xs font-black mb-1.5 text-slate-700">溫層選擇</label>
                    <div className="flex bg-gray-100 p-1 rounded-xl">
                      <button type="button" onClick={() => setFormData({ ...formData, category: "room_temp" })} className={`flex-1 py-1.5 text-xs font-black rounded-lg ${formData.category === "room_temp" ? "bg-[#0058a3] text-white" : "text-gray-500"}`}>常溫</button>
                      <button type="button" onClick={() => setFormData({ ...formData, category: "frozen" })} className={`flex-1 py-1.5 text-xs font-black rounded-lg ${formData.category === "frozen" ? "bg-[#0058a3] text-white" : "text-gray-500"}`}>冷凍</button>
                    </div>
                  </div>
                </div>
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                  <div><label className="block text-xs font-black mb-1.5 text-slate-700">商品名稱</label><input type="text" name="name" required value={formData.name} onChange={handleInputChange} disabled={auth.role !== "admin" && editingId} className="w-full px-3 py-2 border-2 rounded-xl text-sm font-bold focus:border-[#0058a3] outline-none" /></div>
                  <div><label className="block text-xs font-black mb-1.5 text-slate-700">存放地點</label><select name="location" value={formData.location} onChange={handleInputChange} className="w-full px-3 py-2 border-2 rounded-xl text-sm font-bold focus:border-[#0058a3] outline-none"><option value="">請選擇...</option>{locations.map((loc) => (<option key={loc} value={loc}>{loc}</option>))}</select></div>
                </div>
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                  <div><label className="block text-xs font-black mb-1.5 text-slate-700">進貨日期</label><input type="date" name="receiveDate" required value={formData.receiveDate} onChange={handleInputChange} disabled={auth.role !== "admin" && editingId} className="w-full px-3 py-2 border-2 rounded-xl text-sm font-bold focus:border-[#0058a3] outline-none" /></div>
                ...
                <div className="grid grid-cols-3 gap-3">
                  ...
                  <div className="bg-gray-50 border-2 rounded-xl p-2 flex flex-col justify-center">
                    ...
                    {formData.hasSecondReminder ? <input type="number" name="reminderDays2" min="1" required value={formData.reminderDays2} onChange={handleInputChange} disabled={auth.role !== "admin" && editingId} className="w-full px-2 py-1 border border-gray-300 rounded-lg text-xs font-bold outline-none" /> : <div className="text-[10px] text-gray-400 font-bold text-center">未啟用</div>}
                  </div>
                </div>
              </form>
            </div>
            <div className="p-4 bg-gray-50 flex gap-3 justify-end rounded-b-3xl border-t">
              <button type="button" onClick={closeModal} className="px-6 py-3 text-slate-600 bg-white border-2 font-black rounded-xl text-sm">取消</button>
              <button type="submit" form="productForm" className="flex-1 px-6 py-3 bg-[#0058a3] text-[#FBD914] font-black rounded-xl shadow-md flex justify-center items-center gap-2"><Save className="w-5 h-5" /> 儲存資料</button>
            </div>
          </div>
        </div>
      )}
